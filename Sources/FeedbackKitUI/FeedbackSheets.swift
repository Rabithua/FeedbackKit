...
private final class FeedbackComposerModel {
    ...
    init(kind: FeedbackKind, product: FeedbackProduct, client: FeedbackClient, draftStore: FeedbackDraftStore) {
        self.kind = kind; self.product = product; self.client = client; self.draftStore = draftStore
        includesDiagnostics = kind == .bug && product.diagnostics?.supportsSchemaOne == true && client.diagnosticsProvider != nil
        disclosedVisibility = product.defaultFeedbackVisibility
    }

    var diagnosticsAvailable: Bool { product.diagnostics?.supportsSchemaOne == true && client.diagnosticsProvider != nil }
    var canSubmit: Bool { body.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false && isSubmitting == false }
    ...
    func saveDraft() async {
        let draft = FeedbackDraft(productSlug: product.slug, kind: kind, title: title, body: body, includesDiagnostics: includesDiagnostics)
        if draft.isEmpty { try? await draftStore.remove(productSlug: product.slug) }
        ...
        for item in items.prefix(max(0, product.attachmentLimits.count - attachments.count)) {
            do {
                guard let data = try await item.loadTransferable(type: Data.self),
                      let type = item.supportedContentTypes.first(where: { Self.allowedTypes.contains($0.identifier) }),
                      let mime = type.preferredMIMEType
                else { errorMessage = FK.text("feedbackkit.attachment.unsupported"); continue }
                let limit = mime.hasPrefix("video/") ? product.attachmentLimits.videoBytes : product.attachmentLimits.imageBytes
    ...
    func submit(locale: Locale, diagnosticsOverride: Bool? = nil) async -> Bool {
        ...
        guard trimmedBody.isEmpty == false else { errorMessage = FK.text("feedbackkit.composer.body.required"); return false }
        guard trimmedTitle.count <= 240, trimmedBody.count <= 20_000 else { errorMessage = FK.text("feedbackkit.composer.too.long"); return false }
        isSubmitting = true; errorMessage = nil; diagnosticFailure = false
        ...
        do {
            ...
            let ids: [String]
            if let uploadedAttachmentIDs { ids = uploadedAttachmentIDs }
            else { ids = try await client.uploadAttachments(attachments); uploadedAttachmentIDs = ids }
            _ = try await client.submitFeedback(
                ...
                locale: locale,
                attachmentIds: ids,
                includeDiagnostics: diagnosticsOverride ?? includesDiagnostics,
    ...
    private static let allowedTypes: Set<String> = ["public.jpeg", "public.png", "public.heic", "org.webmproject.webp", "com.compuserve.gif", "public.mpeg-4", "com.apple.quicktime-movie"]
}
